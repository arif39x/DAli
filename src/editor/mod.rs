...
impl Editor {
    pub fn draw_view(&mut self, screen_rows: u16, screen_cols: u16) -> Result<(), Box<dyn Error>> {
        if self.show_help_overlay {
            return self.draw_help_overlay(screen_rows, screen_cols);
        }
    ...
    pub fn run(&mut self) -> Result<(), Box<dyn Error>> {
        loop {
            ...
            {
                if self.process_keypress(event)? {
                    break;
                }
            }
        }
    ...
    fn editor_scroll(&mut self, window_idx: usize, cur_row: usize, cur_col: usize) {
        ...
        if cur_row < window.rowoff {
            window.rowoff = cur_row;
        }
        if cur_row >= window.rowoff + viewport.height as usize {
            window.rowoff = cur_row - viewport.height as usize + 1;
        }
        if cur_col < window.coloff {
            window.coloff = cur_col;
        }
        if cur_col >= window.coloff + (viewport.width.saturating_sub(2)) as usize {
            window.coloff = cur_col - (viewport.width.saturating_sub(2)) as usize + 1;
        }
    }

    pub(crate) fn find_index_by_coords(
        &self,
        window_idx: usize,
        target_row: usize,
        target_col: usize,

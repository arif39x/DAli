...
impl Editor {
            ...
            Event::Key(key) => {
                if key.modifiers.contains(KeyModifiers::CONTROL) {
                    match key.code {
                        ...
                        KeyCode::Char('e') => { self.is_command_mode = true; self.command_buffer.clear(); return Ok(false); }
                        KeyCode::Char('c') => { self.copy_selection(); return Ok(false); }
                        KeyCode::Char('x') => { self.cut_selection(); self.focused_window().dirty = true; return Ok(false); }
                        KeyCode::Char('v') => { self.paste_from_clipboard(); self.focused_window().dirty = true; return Ok(false); }
                        _ => {}
                    }
                }
                ...
                match key.code {
                    ...
                    KeyCode::Tab => {
                        if key.modifiers.contains(KeyModifiers::SHIFT) { self.handle_outdent(); }
                        else { self.handle_snippet(); }
                    ...
                    KeyCode::Left | KeyCode::Right | KeyCode::Up | KeyCode::Down => {
                        ...
                        if is_shift && self.windows[idx].selection_start.is_none() {
                            self.windows[idx].selection_start = Some(self.windows[idx].buffer.cursor_pos());
                        } else if !is_shift {
        ...
        match code {
            ...
            KeyCode::Enter => {
                ...
                match tokens[0] {
                    ...
                    "vsplit" | "term" => {
                        let is_term = tokens[0] == "term" || tokens.get(1) == Some(&"term");
                    ...
                    _ => { self.status_msg = format!("ERROR: Unknown command '{}'", cmd_raw); self.status_time = Instant::now(); }
                }
    ...
    pub(crate) fn handle_outdent(&mut self) {
        ...
        if to_remove > 0 {
            ...
            self.windows[f_idx].buffer.move_cursor(cursor_pos.saturating_sub(to_remove));
        }
    }
    ...
    fn outdent_selection(&mut self, start: usize, end: usize) {
        ...
        for r in (start_row..=end_row).rev() {
            ...
            self.windows[f_idx].buffer.move_cursor(line_start + to_remove);
            for _ in 0..to_remove {
                self.windows[f_idx].buffer.delete();
            }
        }
